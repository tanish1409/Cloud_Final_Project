...
# ── I-80 constants ────────────────────────────────────────────────────────────
I80_VALID_LANES = {1, 2, 3, 4, 5, 6}
ONRAMP_LANE_ID = 6
MAINLINE_LANES = {1, 2, 3, 4, 5}
FEET_TO_METRES = 0.3048
FPS_TO_MPS = 0.3048
FRAME_RATE_HZ = 10
SEGMENT_LENGTH_FT = 1650

REQUIRED_COLS = {
    "Vehicle_ID": "int64", "Frame_ID": "int64", "Total_Frames": "int64",
    "Global_Time": "int64", "Local_X": "float64", "Local_Y": "float64",
    "Global_X": "float64", "Global_Y": "float64", "v_Length": "float64",
    "v_Width": "float64", "v_Class": "int64", "v_Vel": "float64",
    "v_Acc": "float64", "Lane_ID": "int64", "Preceding": "int64",
    "Following": "int64", "Space_Headway": "float64", "Time_Headway": "float64",
}
...
def load_raw(filepath):
    ...
    df = pd.read_csv(filepath, sep=r"\s+|,", engine="python", header=0, on_bad_lines="warn")
...
def _sanity_checks(df):
    before = len(df)
    df = df[df["speed_ms"].between(0, 50)]
    df = df[df["accel_ms2"].between(-10, 10)]
