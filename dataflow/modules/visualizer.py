...
CAR_LENGTH_M = 4.5; CAR_WIDTH_M = 2.0
EGO_COLOUR = "#E53935"; LEADER_COLOUR = "#1E88E5"; CUTTER_COLOUR = "#F57C00"; OTHER_COLOUR = "#546E7A"; ROAD_COLOUR = "#0d1117"
SCENARIO_TITLES = {"car_following": "Scenario 1 — Car Following", "onramp_merge": "Scenario 2 — On-Ramp Merge", "lane_cutin": "Scenario 3 — Lane Cut-In"}
SCENE_LABELS = {"car_following": "Ego follows Leader in the same lane", "onramp_merge": "Ego merges from ramp lane → mainline", "lane_cutin": "Cutter cuts in front of Ego"}
...
def _draw_road(ax, y_min, y_max, padding=15):
    ax.set_facecolor(ROAD_COLOUR); ax.set_xlim(y_min - padding, y_max + padding); ax.set_ylim(-1.5, max(LANE_BOUNDARIES) + 1.5)
    for i in range(len(LANE_BOUNDARIES) - 1):
        shade = "#1a2535" if i % 2 == 0 else "#1e2a3a"; ax.axhspan(LANE_BOUNDARIES[i], LANE_BOUNDARIES[i+1], color=shade, zorder=0)
    for i, x in enumerate(LANE_BOUNDARIES):
        style = "-" if i in (0, len(LANE_BOUNDARIES)-1) else "--"; lw = 2.0 if i in (0, len(LANE_BOUNDARIES)-1) else 0.8
    ...
    ax.set_xlabel("Longitudinal Position (m)", color="white", fontsize=9); ax.set_ylabel("Lateral Position (m)", color="white", fontsize=9)
...
def _draw_car(ax, y_c, x_c, colour, label, alpha=1.0, zorder=5):
    ...
    ax.add_patch(rect); ax.text(y_c, x_c, label, color="white", fontsize=6, fontweight="bold", ha="center", va="center", zorder=zorder + 1, path_effects=[pe.withStroke(linewidth=1.5, foreground="black")])

def _draw_trail(ax, y_pos, x_pos, colour, zorder=3):
    if len(y_pos) < 2: return
    ax.plot(y_pos, x_pos, color=colour, linestyle=":", linewidth=1.8, alpha=0.5, zorder=zorder); ax.scatter(y_pos[:-1], x_pos[:-1], color=colour, s=7, alpha=0.35, zorder=zorder)

def _draw_arrow(ax, y_pos, x_pos, colour, zorder=6):
    if len(y_pos) < 2: return
    dy = y_pos[-1] - y_pos[-2]; dx = x_pos[-1] - x_pos[-2]; norm = np.sqrt(dy**2 + dx**2)
    if norm < 0.01: return
    scale = 3.0; tip_y = y_pos[-1] + (dy / norm) * scale; tip_x = x_pos[-1] + (dx / norm) * scale
...
def _subsample(lst, n=15):
    ...
    idx = np.linspace(0, len(lst)-1, n, dtype=int); return [lst[i] for i in idx]
...
def _render_scene(ax, ego_traj, surrounding, highlight_ids, id_colour_map):
    ...
    _draw_road(ax, y_min, y_max); _draw_trail(ax, _subsample(ego_y, 15), _subsample(ego_x, 15), EGO_COLOUR)
    _draw_arrow(ax, ego_y, ego_x, EGO_COLOUR); _draw_car(ax, ego_y[-1], ego_x[-1], EGO_COLOUR, "Ego", zorder=8)
    car_counter = 1
    for sv in surrounding:
        sv_id = sv["vehicle_id"]; sv_y = sv.get("y_m", []); sv_x = sv.get("x_m", [])
        if not sv_y: continue
        colour = id_colour_map.get(sv_id, OTHER_COLOUR); label = highlight_ids.get(sv_id, f"Car {car_counter}"); is_key = sv_id in highlight_ids
        _draw_trail(ax, _subsample(sv_y, 15), _subsample(sv_x, 15), colour); _draw_arrow(ax, sv_y, sv_x, colour)
...
def _style_panel(ax):
    ax.set_facecolor("#111827"); ax.tick_params(colors="white", labelsize=7); ax.grid(True, alpha=0.12, color="white")
    for spine in ax.spines.values(): spine.set_edgecolor("#2a2a2a")

def _panel_car_following(ax, ego_spd, ego_frm, surrounding, leader_id, sample):
    _style_panel(ax); t = [(f - ego_frm[0]) / 10.0 for f in ego_frm]; ax.plot(t, ego_spd, color=EGO_COLOUR, linewidth=2, label="Ego")
    for sv in surrounding:
    ...
    ax.set_title(f"Speed over Time\n{info}", color="white", fontsize=8); ax.set_xlabel("Time (s)", color="white", fontsize=8); ax.set_ylabel("Speed (m/s)", color="white", fontsize=8)
    ax.legend(fontsize=7, facecolor="#1a1a2e", labelcolor="white")

def _panel_onramp_merge(ax, ego_traj, ego_frm, sample):
    _style_panel(ax); t = [(f - ego_frm[0]) / 10.0 for f in ego_frm]; lanes = [r["Lane_ID"] for r in ego_traj]
    ax.step(t, lanes, color="#4CAF50", linewidth=2, where="post"); ax.fill_between(t, lanes, alpha=0.12, color="#4CAF50", step="post")
    ...
    ax.set_title("Lane Transition", color="white", fontsize=8); ax.set_xlabel("Time (s)", color="white", fontsize=8); ax.set_ylabel("Lane", color="white", fontsize=8)
    ax.legend(fontsize=7, facecolor="#1a1a2e", labelcolor="white")
